...
class AWSConfig(BaseModel):
    ...
    max_wait_time: int = Field(
        600, description="Maximum wait time for instance operations"
    )
...
class SSHConfig(BaseModel):
    ...
    strict_host_checking: bool = Field(
        False, description="Enable strict host key checking"
    )
...
class SyncOptions(BaseModel):
    ...
    bandwidth_limit: Optional[str] = Field(
        None, description="Bandwidth limit (--bwlimit)"
    )
    exclude_patterns: List[str] = Field(
...
class SyncConfig(BaseModel):
    ...
    sync_options: SyncOptions = Field(
        default_factory=SyncOptions, description="Rsync options"
    )

    # Sync behavior
    conflict_resolution: ConflictResolution = Field(
        ConflictResolution.NEWER, description="Conflict resolution strategy"
    )
...
class SyncStats(BaseModel):
    ...
    transfer_rate: Optional[float] = Field(
        None, description="Transfer rate in bytes/sec"
    )
...
class SyncStatus(BaseModel):
    ...
    directory_mappings: Dict[str, Dict[str, DirectoryInfo]] = Field(
        default_factory=dict, description="Status of each directory mapping"
    )

    last_sync: Optional[str] = Field(None, description="Last sync timestamp")
    sync_in_progress: bool = Field(
        False, description="Whether sync is currently running"
    )
...
class ProfileConfig(BaseModel):
    ...
    inherits_from: Optional[str] = Field(
        None, description="Parent profile to inherit from"
    )
