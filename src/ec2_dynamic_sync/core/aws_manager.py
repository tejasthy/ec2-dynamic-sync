...
class AWSManager:
    ...
    def get_instance_id(self) -> Optional[str]:
        ...
        if self.config.instance_name:
            try:
                ...
                error_code = e.response.get("Error", {}).get("Code", "Unknown")
                raise AWSConnectionError(
                    f"Failed to find instance by name: {e}", aws_error_code=error_code
                ) from e
    ...
    def get_instance_info(self, instance_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            raise AWSConnectionError(
                f"Failed to get instance information: {e}", aws_error_code=error_code
            ) from e
    ...
    def start_instance(self, instance_id: str) -> bool:
        ...
        try:
            ...
            raise AWSConnectionError(
                f"Failed to start instance: {e}", aws_error_code=error_code
            ) from e
    ...
    def stop_instance(self, instance_id: str) -> bool:
        ...
        try:
            ...
            raise AWSConnectionError(
                f"Failed to stop instance: {e}", aws_error_code=error_code
            ) from e

    def wait_for_state(
        self, instance_id: str, target_state: str, timeout: Optional[int] = None
    ) -> bool:
        ...
        self.logger.info(
            f"Waiting for instance {instance_id} to reach state '{target_state}'..."
        )
        ...
        while time.time() - start_time < timeout:
            ...
            if current_state == target_state:
                self.logger.info(
                    f"Instance {instance_id} reached state '{target_state}'"
                )
                return True
    ...
    def ensure_instance_running(self, instance_id: str) -> Optional[str]:
        ...
        if current_state == "running":
            ...
            if ip:
                ...
                self.logger.warning(
                    f"Instance {instance_id} is running but has no public IP"
                )
                return None

        if current_state == "stopped" and self.config.auto_start_instance:
            self.logger.info(
                f"Instance {instance_id} is stopped, attempting to start..."
            )
            if self.start_instance(instance_id):
                ...
                if ip:
                    self.logger.info(
                        f"Instance {instance_id} started successfully at {ip}"
                    )
                    return ip
                else:
                    self.logger.error(
    ...
    def get_instance_costs(self, instance_id: str) -> Dict[str, Any]:
        ...
        if info:
            return {
                ...
                "estimated_hourly_cost": self._get_estimated_cost(
                    info["instance_type"]
                ),
                "region": self.config.region,
