...
from .models import SyncConfig, SyncResult, SyncMode
...
class SyncOrchestrator:
    ...
    def __init__(self, config: SyncConfig):
        ...
        self.rsync_manager = RsyncManager(config, self.ssh_manager)
    ...
    @classmethod
    def from_config_file(cls, config_path: Optional[str] = None, profile: Optional[str] = None):
    ...
    def prepare_instance(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Instance {self.instance_id} is in state: {instance_state}")
            ...
            if instance_state != 'running':
                if self.config.aws.auto_start_instance:
                    ...
                    self.logger.error("Instance is not running and auto-start is disabled")
                    return False
    ...
    def sync_all_directories(self, mode: SyncMode = SyncMode.BIDIRECTIONAL, dry_run: bool = False) -> Dict[str, Any]:
        """Sync all configured directories."""
        if not self.current_host:
            if not self.prepare_instance():
                return {'overall_success': False, 'error': 'Instance preparation failed'}
        ...
        for mapping in mappings:
            ...
            try:
                if mode == SyncMode.BIDIRECTIONAL:
                    result = self.rsync_manager.sync_bidirectional(self.current_host, mapping, dry_run)
                elif mode == SyncMode.LOCAL_TO_REMOTE:
                    result = self.rsync_manager.sync_local_to_remote(self.current_host, mapping, dry_run)
                elif mode == SyncMode.REMOTE_TO_LOCAL:
                    result = self.rsync_manager.sync_remote_to_local(self.current_host, mapping, dry_run)
                else:
                    result = {'success': False, 'error': f'Unknown sync mode: {mode}'}
                
                results['directories'][mapping.local_path] = result
    ...
    def get_sync_status(self) -> Dict[str, Any]:
        ...
        for mapping in mappings:
            try:
                ...
                status['directories'][mapping.local_path] = info
            except Exception as e:
                self.logger.error(f"Error getting status for {mapping.local_path}: {e}")
                status['directories'][mapping.local_path] = {'error': str(e)}
